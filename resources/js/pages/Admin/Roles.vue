...
const openCreateModal = () => {
    ...
    form.reset();
    modalTitle.value = 'Tambah Role Baru';
...
//----------------------------------------------------------------
const submit = () => {
    const options = { onSuccess: () => closeModal(), onError: () => nameInput.value?.focus() };
    if (isUpdating.value) {
        form.put(route('admin.roles.update', form.id!), options);
    } else {
...
const checkAll = computed({
    get: () => (props.permissions ? form.permissions.length === props.permissions.length : false),
    set: (value: boolean) => {
        form.permissions = value ? props.permissions.slice() : [];
    },
});
...
// DATA LAIN
//----------------------------------------------------------------
const breadcrumbs: BreadcrumbItem[] = [{ title: 'Manajemen Roles', href: route('admin.roles.index') }];
</script>
            ...
            <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
                <div class="overflow-hidden bg-white p-6 shadow-sm sm:rounded-lg dark:bg-gray-800">
                    ...
                    <div class="mb-6 flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
                        <h2 class="text-2xl font-bold text-gray-800 dark:text-gray-200">Manajemen Roles</h2>
                            ...
                            <tbody>
                                ...
                                >
                                    ...
                                    <td class="space-x-2 px-6 py-4 text-right">
                                        <button @click="openEditModal(role)" class="font-medium text-blue-600 hover:underline dark:text-blue-500">
                                            Edit
                                        </button>
                                        <button @click="handleDelete(role)" class="font-medium text-red-600 hover:underline dark:text-red-500">
                                            Hapus
                                        </button>
                                    </td>
                                </tr>
                            ...
                            <div>
                                ...
                                <div class="mt-2 rounded-md border border-gray-200 dark:border-gray-600">
                                    ...
                                    <div class="grid grid-cols-2 gap-2 p-4 sm:grid-cols-3">
                                        <label v-for="permission in permissions" :key="permission" class="flex items-center">
                                            <input
                                                type="checkbox"
                                                :value="permission"
